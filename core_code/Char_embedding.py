...
class CharEmbeddingLayer(Layer):
    ...
    def __init__(self,Flag, context_max_len, question_max_len, **kwargs):
    ...
    def call(self, char_ids_context,char_ids_qn):
        ...
        self.context_char_embs = embedding_ops.embedding_lookup(char_emb_matrix, tf.reshape(char_ids_context,shape=(-1,self.FLAGS.word_max_len)))
        ...
        self.qn_char_embs = embedding_ops.embedding_lookup(char_emb_matrix, tf.reshape(char_ids_qn, shape=(
        -1, self.FLAGS.word_max_len)))
        ...
        self.context_emb_out = self.conv1d(input_=self.context_char_embs, output_size=self.FLAGS.char_out_size,
        ...
        self.context_emb_out = tf.reshape(self.context_emb_out, shape=(-1, self.FLAGS.context_len,
                                                                       self.FLAGS.char_out_size))
        print("Shape context embs after pooling", self.context_emb_out.shape)

        self.qn_emb_out = self.conv1d(input_=self.qn_char_embs, output_size=self.FLAGS.char_out_size,
                                 width=self.FLAGS.window_width, stride=1)
        ...
        self.qn_emb_out = tf.reshape(self.qn_emb_out, shape=(-1, self.FLAGS.question_len, self.FLAGS.char_out_size))
