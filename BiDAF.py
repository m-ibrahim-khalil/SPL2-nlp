...
class BiAttentionLayer(Layer):
    ...
    def Q2C_Attention(self, context):
        ...
        b = K.max(self.attention, axis=-1, )
        b=tf.reshape(b,shape=(766,1))
        context = K.transpose(context)
        H_A = tf.tensordot(context, b,1)
        H_A = tf.reshape(H_A, shape=(self.shape[3],))
        H_A = tf.tile(H_A, [self.shape[2]])
        H_A = tf.reshape(H_A, shape=(self.shape[2], self.shape[3]))
        return H_A

    @tf.function
    def megamerge(self, context, U_A, H_A):
        a=tf.concat([context,U_A],1)
        b=tf.multiply(context,U_A)
        c=tf.multiply(context,H_A)
        G = tf.concat([a,b,c],1)
        return G
    ...
    def call(self, x):
        ...
        self.attention = tf.reshape(self.attention, shape=(self.shape[2],self.shape[2]))
